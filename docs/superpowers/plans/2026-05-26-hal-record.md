# hal-record Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create the `/hal-record` skill that launches Playwright codegen to record browser interactions, parses the output, maps selectors to OUIA constants, and generates a test proposal compatible with `/hal-implement`.

**Architecture:** A single SKILL.md file defines the skill behavior (no TypeScript code needed — the skill is a set of instructions for the AI agent). The skill is registered in `plugin.json`. The skill uses Bash to launch codegen and read files, Read/Grep to map OUIA constants, and AskUserQuestion for the propose-approve loop.

**Tech Stack:** Playwright codegen (CLI), Bash, string/regex parsing (inline in skill instructions)
...
| ------ | ---- | -------------- |
| Create | `.claude-plugin/skills/hal-record/SKILL.md` | Full skill definition |
| Modify | `.claude-plugin/plugin.json` | Register the new skill |

---

### Task 1: Register hal-record in plugin.json

**Files:**

- Modify: `.claude-plugin/plugin.json`

- [ ] **Step 1: Add hal-record entry to plugin.json**

Add the `hal-record` skill entry after `hal-ouia` in the `skills` array:
...
"skills": [
  ...
  {
    "name": "hal-implement",
    "path": "skills/hal-implement/SKILL.md",
    "description": "Implement new halOP tests interactively with propose-approve-implement loop"
  },
  {
    "name": "hal-ouia",
    "path": "skills/hal-ouia/SKILL.md",
    "description": "Add missing OUIA IDs to halOP for testability, create PRs, and sync back to dave"
  },
  {
    "name": "hal-record",
    "path": "skills/hal-record/SKILL.md",
...
git commit -m "chore: register hal-record skill in plugin.json"
```
...
- Create: `.claude-plugin/skills/hal-record/SKILL.md`

- [ ] **Step 1: Create the skill directory**

Run: `mkdir -p .claude-plugin/skills/hal-record`

- [ ] **Step 2: Write the frontmatter and overview sections**

Create `.claude-plugin/skills/hal-record/SKILL.md` with the frontmatter, title, tools, arguments, and constants sections:
...
```bash
git add .claude-plugin/skills/hal-record/SKILL.md
git commit -m "feat: add hal-record SKILL.md with frontmatter and overview"
```

---

### Task 3: Add Dev Environment Check (Phase 1)

**Files:**

- Modify: `.claude-plugin/skills/hal-record/SKILL.md`

- [ ] **Step 1: Append Phase 1 — Prerequisites Check**
...
```bash
git add .claude-plugin/skills/hal-record/SKILL.md
git commit -m "feat(hal-record): add prerequisites check phase"
```

---

### Task 4: Add Codegen Launch (Phase 2)

**Files:**

- Modify: `.claude-plugin/skills/hal-record/SKILL.md`

- [ ] **Step 1: Append Phase 2 — Launch Codegen**
...
```bash
git add .claude-plugin/skills/hal-record/SKILL.md
git commit -m "feat(hal-record): add codegen launch phase"
```

---

### Task 5: Add Recording Parsing (Phase 3)

**Files:**

- Modify: `.claude-plugin/skills/hal-record/SKILL.md`

- [ ] **Step 1: Append Phase 3 — Parse Recording**
...
```bash
git add .claude-plugin/skills/hal-record/SKILL.md
git commit -m "feat(hal-record): add recording parsing phase"
```

---

### Task 6: Add Proposal Generation (Phase 4)

**Files:**

- Modify: `.claude-plugin/skills/hal-record/SKILL.md`

- [ ] **Step 1: Append Phase 4 — Generate Proposal**
...
```bash
git add .claude-plugin/skills/hal-record/SKILL.md
git commit -m "feat(hal-record): add proposal generation phase"
```

---

### Task 7: Add Approval and Handoff (Phase 5)

**Files:**

- Modify: `.claude-plugin/skills/hal-record/SKILL.md`

- [ ] **Step 1: Append Phase 5 — Approval and Handoff**
...
```bash
git add .claude-plugin/skills/hal-record/SKILL.md
git commit -m "feat(hal-record): add approval and handoff phase"
```

---

### Task 8: Add Error Handling and Anti-Patterns
...
```bash
git add .claude-plugin/skills/hal-record/SKILL.md
git commit -m "feat(hal-record): add error handling and anti-patterns"
```

---

### Task 9: Final Review and Integration Test

**Files:**

- Read: `.claude-plugin/skills/hal-record/SKILL.md` (final)
- Read: `.claude-plugin/plugin.json` (verify)

- [ ] **Step 1: Read the complete SKILL.md to verify structure**

Read the full `.claude-plugin/skills/hal-record/SKILL.md` and verify:

- Frontmatter has name, description, metadata.version
- All 5 phases are present (Prerequisites, Launch Codegen, Parse Recording, Generate Proposal, Approval & Handoff)
- Tools section lists Bash, Read, Grep, AskUserQuestion
- Constants section has all required values
- Error Handling table is complete
- Anti-Patterns section is present

- [ ] **Step 2: Verify plugin.json has 5 skills**

Run: `node -e "const p=JSON.parse(require('fs').readFileSync('.claude-plugin/plugin.json','utf8'));console.log(p.skills.length + ' skills');console.log(p.skills.map(s=>s.name).join(', '))"`

Expected: `5 skills` and `hal-dev-env, hal-explore, hal-implement, hal-ouia, hal-record`

- [ ] **Step 3: Verify SKILL.md follows same structure as existing skills**

Compare section headers against `hal-ouia/SKILL.md` pattern:

```bash
echo "=== hal-record sections ==="
grep "^##" .claude-plugin/skills/hal-record/SKILL.md

echo ""
echo "=== hal-ouia sections (reference) ==="
grep "^##" .claude-plugin/skills/hal-ouia/SKILL.md
```

Both should have: title, Tools, Arguments, Constants, phase sections, Error Handling, Anti-Patterns.

- [ ] **Step 4: Verify skill file is under 800 lines**

Run: `wc -l .claude-plugin/skills/hal-record/SKILL.md`
...
```bash
git add .claude-plugin/skills/hal-record/SKILL.md
git commit -m "chore: finalize hal-record skill"
```
